...
<div id="mbhead3">
                    <div id="mbh3leftcontrols">
                        <input id="all_users" name="selected[]" value="all" type="checkbox">
    ...
    <table id="appstable" class="table table-striped">
		<?for($i=0;$i<count($vars);$i++):?>
			<tr id="<?=$vars[$i]['id']?>" class="users_select">
				<td>
					<input class="selected users_select" type="checkbox" name="selected[]" value="<?=$vars[$i]['id']?>"/>
					<img src="<?=BASE_URL?><?=$vars[$i]['icon']?>">
				</td>
				<td class="tdtitle"><?=$vars[$i]['username']?></td>
				<td id="strmenu" style="display:none;" colspan="3">
					<img id="bluearrow" src="v/img/arrow.png">
                    <div id="strmenublock" class="users_select" data-item="<?=$vars[$i]['id']?>">
				...
				<td class="tdautor"><?=$vars[$i]['email']?></td>
				<td class="tdos"><?=$vars[$i]['type_name']?></td>
				<td class="tdate"><?=$vars[$i]['apps']?></td>
			</tr>
		<?endfor?>
	</table>
</div>